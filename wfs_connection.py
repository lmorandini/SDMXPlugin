...
class WFSConnection(object):
    ...
    def encode(self):
        ...
        return base64.b64encode(CONNECTION_SEP.join((self.url, self.username, self.password)).encode('utf-8')).decode(
            'utf-8')
    ...
    def connect(self):
        try:
            resp = requests.get(self.url, auth=self.auth,
                                params={"request": "GetCapabilities", "version": WFS_VERSION, "service": "wfs"})
        ...
        self.cubes = [Cube(m2.group(1), m2.group(2), m2.group(0)) for m2 in
                      [m for m in [self.cubeRe.search(e.text) for e in self.featureTypes] if m is not None]]

        # Extract cube dimensions
        for cube in self.cubes:
            try:
                resp = requests.get(self.url, auth=self.auth,
                                    params={"request": "GetFeature", "version": WFS_VERSION, "service": "wfs",
            ...
            self.dimensions[cube.featureType] = [Dimension(feat[0].text, feat[1].text, cube.name, cube.dimFeatureType)
    ...
    def getFeatureURL(self, featureType, cqlExpr):
        ...
        req = requests.Request("GET", self.url, params={"request": "GetFeature", "version": WFS_VERSION,
                                                        "service": "wfs", "outputFormat": "csv",
                                                        "typeName": featureType, "cql_filter": cqlExpr})
        proxies = dict()
    ...
    def executeGetFeatureRequest(self, featureType, cqlExpr):
        try:
            resp = requests.get(self.url, auth=self.auth,
                                params={"request": "GetFeature", "version": WFS_VERSION, "service": "wfs",
                                        "outputFormat": "csv",
