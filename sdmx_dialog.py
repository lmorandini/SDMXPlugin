...
class SDMXPluginDialog(QDialog, FORM_CLASS):
    ...
    def exprShown(self):
        if self.activeCube != None:
            ...
            self.cqlExpr.setText(unquote(cqlExpr))

    def executeWFSRequest(self):
        response = self.activeWfsConn.executeGetFeatureRequest(self.activeCube.featureType, self.cqlExpr.toPlainText())
        tmpCsv = tempfile.NamedTemporaryFile(mode='w', suffix='.csv', prefix='sdmx-', delete=False)
        tmpCsv.write(response.text)
        tmpCsv.close()
        table = QgsVectorLayer(Path(tmpCsv.name).as_uri(), 'SDMX:: ' + self.cqlExpr.toPlainText(), 'delimitedtext')
